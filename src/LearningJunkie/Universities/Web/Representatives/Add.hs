...
module LearningJunkie.Universities.Web.Representatives.Add where
...
type API =
    Summary "Add a university representative to a university"
        :> JWTAuth
...
handler universityId (Authenticated authUser) newInstructor =
    requirePermissionWithId
        ...
        (Just universityId)
        $ do
            toUserType <$> insertUser newInstructor Role.UniversityRep (Just universityId)
